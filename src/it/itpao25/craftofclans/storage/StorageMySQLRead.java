...
public class StorageMySQLRead {
	...
	public static HashMap<String, HashMap<String, String>> getPlayers() {
		...
		try {
			PreparedStatement s = (PreparedStatement) StorageMySQL.con.prepareStatement(query);
	...
	/**
	 * Controllo se l'utente è presente nel database MySQL
	 */
	...
	/**
	 * Registro l'utente nel database MySQL
	 */
	...
	/**
	 * faccio il get dell'utente owner (id)
	 */
	...
	public static boolean VillosetDataAttacco(int x, int z) {
		String query = "UPDATE `craftofclans_villages` SET data_last_attack = NOW() WHERE `pointX`=(?) AND `pointZ`=(?)";
		try {
			PreparedStatement s = (PreparedStatement) StorageMySQL.con.prepareStatement(query);
			s.setLong(1, x);
			s.setLong(2, z);
	...
	public static boolean createClan(String name, String desc, ClansTypes type, int min_trophies, UUID id) {
		...
		try {
			PreparedStatement s = (PreparedStatement) StorageMySQL.con.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
			s.setInt(1, id_user);
			s.setString(2, name);
			s.setString(3, desc);
			...
			if (rs.next()) {
				int id_last = rs.getInt(1);

				// Aggiorno lo stato del giocatore
				String query2 = "UPDATE `craftofclans_users` SET id_clan=(?) WHERE `id`=(?)";
				PreparedStatement s2 = (PreparedStatement) StorageMySQL.con.prepareStatement(query2);
				s2.setInt(1, id_last);
				s2.setInt(2, id_user);
				s2.execute();
				s.close();
	...
	/**
	 * Aggiorno l'ultimo login dell'utente
	 */
	public static boolean updateLastLogin(UUID id) {

		int id_user = getuserId(id);
		String query = "UPDATE `craftofclans_users` SET last_login = NOW() WHERE id=(?)";
	...
	public static void setPlayerClan(UUID player, Integer id_clan) {
		...
		String query = "UPDATE `craftofclans_users` SET `id_clan` = (?), `last_update_clan` = NOW()  WHERE `id` = (?)";
		try {
			PreparedStatement s = (PreparedStatement) StorageMySQL.con.prepareStatement(query);
			s.setInt(1, id_clan);
			s.setInt(2, id_user);
	...
	/**
	 * Prendo la data dell'ultimo attacco
	 */
	...
	public static boolean registroAttacco(UUID id, VillageId villo, double gems_won, double gold_won, double elixir_won, double darkelixir_won, int percentage, int trofei_vinti, int trofei_persi) {
		...
		String query = "INSERT INTO `craftofclans_attacks` (`id`, `village`, `id_attacker`, `gems_won`, `gold_won`, `elixir_won`, " + "`darkelixir_won`, `percentage`, `trophies_won`, `trophies_lost`, `data_attack`) VALUES " + "(NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW());";
