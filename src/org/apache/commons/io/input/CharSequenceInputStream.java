...
public class CharSequenceInputStream extends InputStream {
	 ...
	 * @throws IllegalArgumentException if the buffer is not large enough to hold a
	 *                                  complete character
	 */
	...
	/**
	 * Constructor, calls
	 * {@link #CharSequenceInputStream(CharSequence, Charset, int)} with a buffer
	 * size of 2048.
	 *
	 * @param cs      the input character sequence
	 * @param charset the character set name to use
	...
	public long skip(long n) throws IOException {
		/*
		 * This could be made more efficient by using position to skip within the
		 * current buffer.
		 */
		long skipped = 0;
	 ...
	 * 
	 * @return the count of bytes that can be read without blocking (or returning
	 *         EOF).
	 *
	...
	public int available() throws IOException {
		// The cached entries are in bbuf; since encoding always creates at least one
		// byte
		// per character, we can add the two to get a better estimate (e.g. if bbuf is
		// empty)
		// Note that the previous implementation (2.4) could return zero even though
		// there were
	...
	public synchronized void reset() throws IOException {
		/*
		 * This is not the most efficient implementation, as it re-encodes from the
		 * beginning.
		 *
		 * Since the bbuf is re-used, in general it's necessary to re-encode the data.
		 *
		 * It should be possible to apply some optimisations however: + use mark/reset
		 * on the cbuf and bbuf. This would only work if the buffer had not been
		 * (re)filled since the mark. The code would have to catch InvalidMarkException
		 * - does not seem possible to check if mark is valid otherwise. + Try saving
		 * the state of the cbuf before each fillBuffer; it might be possible to restart
		 * from there.
		 */
