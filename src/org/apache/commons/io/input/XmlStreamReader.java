...
public class XmlStreamReader extends Reader {
	...
	private static final ByteOrderMark[] XML_GUESS_BYTES = new ByteOrderMark[] { new ByteOrderMark(UTF_8, 0x3C, 0x3F, 0x78, 0x6D), new ByteOrderMark(UTF_16BE, 0x00, 0x3C, 0x00, 0x3F), new ByteOrderMark(UTF_16LE, 0x3C, 0x00, 0x3F, 0x00),
			new ByteOrderMark(UTF_32BE, 0x00, 0x00, 0x00, 0x3C, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x78, 0x00, 0x00, 0x00, 0x6D), new ByteOrderMark(UTF_32LE, 0x3C, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x78, 0x00, 0x00, 0x00, 0x6D, 0x00, 0x00, 0x00),
	...
	/**
	 * Returns the default encoding to use if none is set in HTTP content-type, XML
	 ...
	 * <p>
	 * It looks for the UTF-8 BOM first, if none sniffs the XML prolog charset, if
	 * this is also missing defaults to UTF-8.
	 * <p>
	 * It does a lenient charset encoding detection, check the constructor with the
	 * lenient parameter for details.
	 *
	 ...
	 *                                  stream.
	 * @throws XmlStreamReaderException thrown if the charset encoding could not be
	 ...
	 * <p>
	 * If the URL is not of type HTTP and there is not 'content-type' header in the
	 * fetched data it uses the same logic used for Files.
	 * <p>
	 ...
	 * @throws IOException thrown if there is a problem reading the stream of the
	 *                     URL.
	 */
	 ...
	 * files.
	 * <p>
	 * If the URLConnection is a HTTP Url or there is a 'content-type' header in the
	 ...
	 * content-type encoding. If there is not content-type encoding checks the XML
	 * prolog encoding. If there is not XML prolog encoding uses the default
	 ...
	 * @param httpContentType content-type header to use for the resolution of the
	 *                        charset encoding.
	 * @throws IOException thrown if there is a problem reading the file.
	 */
	...
	/**
	 * Creates a Reader using an InputStream an the associated content-type header.
	 * This constructor is lenient regarding the encoding detection.
	 * <p>
	 * First it checks if the stream has BOM. If there is not BOM checks the
	 * content-type encoding. If there is not content-type encoding checks the XML
	 * prolog encoding. If there is not XML prolog encoding uses the default
	...
	/**
	 * Returns charset parameter value, NULL if not present, NULL if httpContentType
	 * is NULL.
	 *
	 * @param httpContentType the HTTP content type
