 ...
 * encoding. The stream is transformed using a {@link CharsetEncoder} object,
 * guaranteeing that all charset encodings supported by the JRE are handled
 ...
 * <p>
 * Since in general it is not possible to predict the number of characters to be
 * read from the {@link Reader} to satisfy a read request on the
 * {@link ReaderInputStream}, all reads from the {@link Reader} are buffered.
 * There is therefore no well defined correlation between the current position
 * of the {@link Reader} and that of the {@link ReaderInputStream}. This also
 * implies that in general there is no need to wrap the underlying
 * {@link Reader} in a {@link java.io.BufferedReader}.
 * <p>
 * {@link ReaderInputStream} implements the inverse transformation of
 * {@link java.io.InputStreamReader}; in the following example, reading from
 * {@code in2} would return the same byte sequence as reading from {@code in}
 * (provided that the initial byte sequence is legal with respect to the charset
 * encoding):
 * 
 * <pre>
 * InputStream in = ...
 ...
 * {@link java.io.OutputStreamWriter} pushes data to the underlying stream,
 ...
 * an existing API only accepts an {@link InputStream}, but where the most
 * natural way to produce the data is as a character stream, i.e. by providing a
 * {@link Reader} instance. An example of a situation where this problem may
 * appear is when implementing the {@link javax.activation.DataSource} interface
 ...
 * method. A call to this method will always return 0. Also, this class doesn't
...
public class ReaderInputStream extends InputStream {
	...
	/**
	 * CharBuffer used as input for the decoder. It should be reasonably large as we
	 * read data from the underlying Reader into this buffer.
	 */
	private final CharBuffer encoderIn;

	/**
	 * ByteBuffer used as output for the decoder. This buffer can be small as it is
	 * only used to transfer data from the decoder to the buffer provided by the
	 * caller.
	 */
	...
	/**
	 * Construct a new {@link ReaderInputStream} that uses the default character
	 * encoding with a default input buffer size of 1024 characters.
	 *
	 * @param reader the target {@link Reader}
	 ...
	 * Read a single byte.
	 *
	 * @return either the byte read or <code>-1</code> if the end of the stream has
	 *         been reached
	 * @throws IOException if an I/O error occurs
	 */
	@Override
	...
	/**
	 * Close the stream. This method will cause the underlying {@link Reader} to be
	 * closed.
	 * 
	 * @throws IOException if an I/O error occurs
	 */
