 ...
 * 
 * <pre>
 * BOMInputStream bomIn = new BOMInputStream(in, ByteOrderMark.UTF_16LE, ByteOrderMark.UTF_16BE, ByteOrderMark.UTF_32LE, ByteOrderMark.UTF_32BE);
...
public class BOMInputStream extends ProxyInputStream {
	...
	/**
	 * Constructs a new BOM InputStream that excludes a {@link ByteOrderMark#UTF_8}
	 * BOM.
	 * 
	...
	/**
	 * Constructs a new BOM InputStream that detects a a {@link ByteOrderMark#UTF_8}
	...
	public BOMInputStream(final InputStream delegate, final boolean include, final ByteOrderMark... boms) {
		...
		// Sort the BOMs to match the longest BOM first because some BOMs have the same
		// starting two bytes.
	 ...
	 * 
	 * @return true if the stream has one of the specified BOMs, otherwise false if
	 *         it does not
	 ...
	 * @return true if the stream has the specified BOM, otherwise false if it does
	 *         not
	 * @throws IllegalArgumentException if the BOM is not one the stream is
	 *                                  configured to detect
	 * @throws IOException              if an error reading the first bytes of the
	 *                                  stream occurs
	 */
	 ...
	 * stream. It behaves like the single-byte <code>read()</code> method, either
	 * returning a valid byte or -1 to indicate that the initial bytes have been
	 * processed already.
	 * 
	...
	/**
	 * Invokes the delegate's <code>read()</code> method, detecting and optionally
	 * skipping BOM.
	 * 
	 * @return the byte read (excluding BOM) or -1 if the end of stream
	 * @throws IOException if an I/O error occurs
