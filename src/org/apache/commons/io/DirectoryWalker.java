...
public abstract class DirectoryWalker<T> {
	 ...
	 * <p>
	 * The filter controls which files and directories will be navigated to as part
	 * of the walk. The {@link FileFilterUtils} class is useful for combining
	 * various filters together. A {@code null} filter means that no filtering
	 * should occur and all files and directories will be visited.
	 *
	 ...
	 * of the walk. This constructor uses
	 * {@link FileFilterUtils#makeDirectoryOnly(IOFileFilter)} and
	 * {@link FileFilterUtils#makeFileOnly(IOFileFilter)} internally to combine the
	 * filters. A {@code null} filter means that no filtering should occur.
	 *
	 * @param directoryFilter the filter to apply to directories, null means visit
	 *                        all directories
	 ...
	 * Once called, this method will emit events as it walks the hierarchy. The
	 * event methods have the prefix <code>handle</code>.
	 *
	 ...
	 * typically {@link #handleFile}, may take a long time to run. In that case, you
	 * may wish to check for cancellation by calling this method.
	 *
	...
	/**
	 * Overridable callback method invoked to determine if the entire walk operation
	 * should be immediately cancelled.
	 * <p>
	 * This method should be implemented by those subclasses that want to provide a
	 * public <code>cancel()</code> method available from another thread. The design
	 * pattern for the subclass should be as follows:
	 * 
	 * <pre>
	 * public class FooDirectoryWalker extends DirectoryWalker {
	 * 	private volatile boolean cancelled = false;
	...
	/**
	 * Overridable callback method invoked when the operation is cancelled. The file
	 * being processed when the cancellation occurred can be obtained from the
	 * exception.
	 * <p>
	 ...
	 * or not. If you return false, the entire directory and any subdirectories will
	 * be skipped. Note that this functionality is in addition to the filtering by
	 * file filter.
	 * <p>
	 ...
	 * @param directory the current directory being processed
	 * @param depth     the current directory level (starting directory = 0)
	 * @param files     the files (possibly filtered) in the directory, may be
	 *                  {@code null}
	...
	public static class CancelException extends IOException {
		...
		/**
		 * Constructs a <code>CancelException</code> with the file and depth when
		...
		/**
		 * Constructs a <code>CancelException</code> with an appropriate message and the
		 * file and depth when cancellation occurred.
		 *
