...
public class HexDump {
	...
	/**
	 * Dump an array of bytes to an OutputStream. The output is formatted for human
	 * inspection, with a hexadecimal offset followed by the hexadecimal values of
	 * the next 16 bytes of data and the printable ASCII characters (if any) that
	 * those bytes represent printed per each line of output.
	 * <p>
	 ...
	 * array contains the third kibibyte of a file, then the offset argument should
	 * be set to 2048. The offset value printed at the beginning of each line
	 * indicates where in that larger entity the first byte on that line is located.
	 * <p>
	 * All bytes between the given index (inclusive) and the end of the data array
	 * are dumped.
	 *
	 ...
	 *
	 * @throws IOException                    is thrown if anything goes wrong
